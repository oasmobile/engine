...
function searchAtlas (skeletonPath, callback) {
    ...
    function next (index) {
        ...
        if (index < 1)
        {
            ...
            path = skeletonPath + suffix;
        }
        else
        {
            path = skeletonPath.substr(0, skeletonPath.lastIndexOf('/'));
            path = path + "/" + path.substr(path.lastIndexOf('/') + 1, path.length - path.lastIndexOf('/')) +".atlas";
        }
        ...
        Fs.exists(path, exists => {
            if (exists) {
                ...
                callback(new Error(`Can not find ${skeletonPath + ATLAS_EXTS[0]} | ${atlasPath}`));
            }
        });
    }
